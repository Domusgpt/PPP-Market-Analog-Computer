...
export class TopologyController {
    ...
    private _cell24Provider: TopologyProvider | null = null;
    ...
    private _getCell24Provider(): TopologyProvider {
        if (!this._cell24Provider) {
            this._cell24Provider = buildCell24Provider();
        }
        return this._cell24Provider;
    }
}

export const defaultTopologyController = new TopologyController();

function buildCell24Provider(): TopologyProvider {
    const lattice = getDefaultLattice();
    const vertices = lattice.vertices.map(vertex => vertex.coordinates);
    const neighbors = lattice.vertices.map(vertex => vertex.neighbors);

    return {
        name: 'Lattice24',
        vertices,
        neighbors,
        circumradius: lattice.circumradius,
        checkConvexity: (point: Vector4D, kNearest?: number) =>
            lattice.checkConvexity(point, kNearest),
        computeCoherence: (point: Vector4D, kNearest?: number) =>
            lattice.checkConvexity(point, kNearest).coherence
    };
}
