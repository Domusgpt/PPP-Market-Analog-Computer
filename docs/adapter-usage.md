...
### Adapter Registry (Multi-Source)
```ts
import { AdapterRegistry, MarketQuoteAdapter } from '../src/lib';

const registry = new AdapterRegistry();
registry.register('alpaca', new MarketQuoteAdapter({ source: 'alpaca-rest' }));
registry.register('polygon', new MarketQuoteAdapter({ source: 'polygon-ws' }));

console.log(registry.list()); // ['alpaca', 'polygon']

registry.forEach((adapter, name) => {
  console.log(name, adapter.metrics?.());
});

registry.unregister('polygon');
```
...
- Sample runnable script: `samples/adapter-market-quote.ts` (run with `npx tsx samples/adapter-market-quote.ts`) for a full end-to-end ingest → frame snapshot.
