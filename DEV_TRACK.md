...
## Session 49 – Observability schema/version contract
- Timestamp (UTC): 2026-02-21T04:35:22Z
- Added stream observability schema metadata (`schema_version`, `generated_at`) for `get_statistics()` and snapshot consumers.
- Added `snapshot_kind` to `get_observability_snapshot()` to support explicit payload typing in monitoring pipelines.
- Extended integration tests to assert schema/timestamp fields for legacy and quasicrystal paths.
