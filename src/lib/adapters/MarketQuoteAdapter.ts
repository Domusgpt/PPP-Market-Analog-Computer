...
export interface MarketQuoteFieldMap {
  symbol: string;
  bid?: string;
  ask?: string;
  last?: string;
...
export class MarketQuoteAdapter implements PPPAdapter {
  ...
  ingestFrom(record: Record<string, unknown>, map: MarketQuoteFieldMap): RawApiTick {
    ...
    const symbolValue = record[map.symbol];
    const symbol = typeof symbolValue === 'string' ? symbolValue : String(symbolValue ?? '');

    return this.ingestQuote({
      symbol,
      bid: pickNumber(map.bid),
      ask: pickNumber(map.ask),
      last: pickNumber(map.last),
      volume: pickNumber(map.volume),
      bidSize: pickNumber(map.bidSize),
      askSize: pickNumber(map.askSize),
      timestamp: pickNumber(map.timestamp)
    });
  }
