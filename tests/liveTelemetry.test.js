...
test('appendLiveStatusLog enforces TTL and recalculates latest status metadata', () => {
    const originalNow = Date.now;
    ...
    try {
        ...
        appendLiveStatusLog(
            state,
            { message: 'stale status', level: 'warning', at: fixedNow - 6_000 },
            { limit: 5, ttlMs: 5_000 }
        );

        appendLiveStatusLog(state, { message: 'fresh status', level: 'info', at: fixedNow - 1_000 });

        assert.equal(state.statusTtlMs, 5_000);
        assert.equal(state.statusLog.length, 1);
        assert.equal(state.lastStatus, 'fresh status');
        assert.equal(state.lastStatusLevel, 'info');
        assert.equal(state.lastStatusAt, fixedNow - 1_000);
        assert.match(state.telemetrySummary, /status info: fresh status/);
    } finally {
        Date.now = originalNow;
    }
});
...
test('mergeLiveAdapterMetrics enforces adapter status TTLs and clears stale entries', () => {
    const originalNow = Date.now;
    ...
    try {
        const state = createLiveStreamState();
        mergeLiveAdapterMetrics(state, {
            status: { message: 'old status', level: 'warning', at: fixedNow - 5_000, ttlMs: 2_000 },
            connected: true
        });

        assert.equal(state.statusLog.length, 0);
        assert.equal(state.lastStatus, null);
        assert.equal(state.lastStatusLevel, null);
        assert.equal(state.statusTtlMs, 2_000);
        assert.doesNotMatch(state.telemetrySummary, /status /);

        mergeLiveAdapterMetrics(state, {
            status: { message: 'new status', level: 'info', at: fixedNow - 500 },
            connected: true
        });

        assert.equal(state.statusLog.length, 1);
        assert.equal(state.lastStatus, 'new status');
        assert.equal(state.lastStatusLevel, 'info');
        assert.match(state.telemetrySummary, /status info: new status/);
    } finally {
        Date.now = originalNow;
    }
});
...
test('staleness tracking surfaces in health and summaries', () => {
    const originalNow = Date.now;
    const fixedNow = 1_700_000_000_000;
    Date.now = () => fixedNow;
    try {
        ...
        Object.assign(state, {
            connected: true,
            frames: 3,
            lastReceiveAt: fixedNow - 6_000,
            staleAfterMs: 5_000,
            staleCriticalAfterMs: 12_000
        });

        const warningHealth = evaluateLiveHealth(state);
        assert.equal(warningHealth.level, 'warning');
        assert.ok(warningHealth.reasons.includes('stale'));

        const summary = formatLiveTelemetrySummary(state);
        assert.match(summary, /stale 6s/);

        state.lastReceiveAt = fixedNow - 20_000;
        const errorHealth = evaluateLiveHealth(state);
        assert.equal(errorHealth.level, 'error');
        assert.ok(errorHealth.reasons.includes('stale-critical'));
    } finally {
        Date.now = originalNow;
    }
});

test('mergeLiveAdapterMetrics ingests staleness metadata and refreshes summary', () => {
    const originalNow = Date.now;
    const fixedNow = 1_700_000_050_000;
    Date.now = () => fixedNow;
    try {
        const state = createLiveStreamState();
        mergeLiveAdapterMetrics(state, {
            lastReceiveAt: fixedNow - 7_000,
            staleAfterMs: 4_000,
            staleCriticalAfterMs: 9_000,
            connected: true
        });

        assert.equal(state.lastReceiveAt, fixedNow - 7_000);
        assert.equal(state.staleAfterMs, 4_000);
        assert.equal(state.staleCriticalAfterMs, 9_000);
        assert.match(state.telemetrySummary, /stale 7s/);
        assert.equal(state.health.level, 'warning');
    } finally {
        Date.now = originalNow;
    }
