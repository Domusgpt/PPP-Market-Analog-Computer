...
test('geometric evidence links hashes and preserves canonical payloads', () => {
    ...
    assert.equal(verifyEvidenceHash(extendedChain[0]), true);
    assert.equal(verifyEvidenceChain(extendedChain), true);
});

test('evidence chain verification detects tampering', () => {
    const chain = appendGeometricEvidence([], {
        eventType: 'GEOMETRIC_STATE',
        polytopalState: { vertices: [[0, 0]] },
        timestamp: 1_700_000_000_000
    });
    const extendedChain = appendGeometricEvidence(chain, {
        eventType: 'COHERENCE_CHECK',
        polytopalState: { vertices: [[1, 1]] },
        timestamp: 1_700_000_000_500
    });
    extendedChain[1].payload.vertices[0][0] = 99;
    assert.equal(verifyEvidenceHash(extendedChain[1]), false);
    assert.equal(verifyEvidenceChain(extendedChain), false);
});
