...
test('trims chains and batches when limits are provided', () => {
    const session = createGeometricAuditSession({ batchSize: 1, maxChainLength: 2, maxBatches: 1 });
    session.appendTelemetry({ polytopalState: { vertices: [[0, 0]] }, timestamp: 1 });
    session.appendTelemetry({ polytopalState: { vertices: [[1, 0]] }, timestamp: 2 });
    session.appendTelemetry({ polytopalState: { vertices: [[2, 0]] }, timestamp: 3 });

    const state = session.getState();
    assert.equal(state.chain.length, 2);
    assert.equal(state.batches.length, 1);
    assert.equal(state.chainHeadHash !== null, true);
    assert.equal(session.verifyChainIntegrity(), true);
    assert.equal(state.batchOffset, 2);
});
