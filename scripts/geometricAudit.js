...
const stripEventHash = (evidence) => {
    if (!evidence || typeof evidence !== 'object') {
        return null;
    }
    // eslint-disable-next-line no-unused-vars
    const { eventHash, ...rest } = evidence;
    return rest;
};

const buildEvidenceHash = (evidence) => {
    const payload = stripEventHash(evidence);
    if (!payload) {
        return null;
    }
...
export const createGeometricEvidence = (input = {}) => {
    ...
    evidence.eventHash = buildEvidenceHash(evidence);
    return evidence;
};
...
export const verifyEvidenceHash = (evidence) => {
    if (!evidence || typeof evidence.eventHash !== 'string') {
        return false;
    }
    const expected = buildEvidenceHash(evidence);
    return expected === evidence.eventHash;
};

export const verifyEvidenceChain = (chain = [], options = {}) => {
    if (!Array.isArray(chain)) {
        return false;
    }
    const headHash = options && typeof options.headHash === 'string' ? options.headHash : null;
    for (let index = 0; index < chain.length; index += 1) {
        const evidence = chain[index];
        if (!verifyEvidenceHash(evidence)) {
            return false;
        }
        const expectedPrevious = index === 0 ? headHash : chain[index - 1].eventHash;
        if (evidence.previousHash !== expectedPrevious) {
            return false;
        }
    }
    return true;
